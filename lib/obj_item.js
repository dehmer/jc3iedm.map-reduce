...
module.exports = async streams => {
  ...
  const merge_oiloca_rec = (coia_recs, data) => {
    ...
    const pk = `${data.contxt_id}|${data.obj_item_id}`
    coia_recs[pk].OILOCA = loc_recs[loc_id]
    if (data.value[2]) coia_recs[pk].brng_angle = data.value[2]
    if (data.value[8]) coia_recs[pk].speed_rate = data.value[8]
    return coia_recs
  }
