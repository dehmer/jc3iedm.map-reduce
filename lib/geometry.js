...
module.exports = async streams => {
  ...
  const geometry = (acc, data) => {
    const coordinates = point => acc[point].coordinates
    const pk = data.key.split(':')[2]

    const mappers = {
      10000220: row => acc[row[0]],
      10000282: row => ({ type: 'Point', coordinates: [row[1], row[0]] }),
      10000061: ()  => ({ type: 'LineString', coordinates: line_points[pk].map(coordinates) }),
      10000218: row => ({ type: 'Polygon', coordinates: [line_points[row[0]].map(coordinates)] }),
      10000214: row => ({ type: 'LineString', coordinates: line_points[row[1]].map(coordinates), geometry_width: row[0] }),
      10000048: row => ({
        type: 'Point',
